...
- Antes de realizar alterações significativas no código, consultar os documentos em `docs/` e em `openspec/`.

## Frontend
- O frontend do projeto deve ser implementado exclusivamente com Reflex.
- Utilizar os mecanismos próprios do Reflex para componentes, estado (State), eventos e páginas.
- Não introduzir outra tecnologia ou framework de frontend sem alteração arquitetural aprovada.

## Desenvolvimento e Código
- As alterações funcionais devem utilizar o fluxo do OpenSpec.
- Reutilizar o código existente e evitar duplicação de funções.
- Não alterar funcionalidades não relacionadas com a tarefa atual sem justificativa.